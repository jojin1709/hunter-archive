...
export async function getWriteup(id) {
  ...
  // Sanitize path traversal, null bytes, and malicious control characters
  const cleanId = id.replaceAll("\0", "").replaceAll("..", "").trim();
  const rows = await readWriteups();
  if (!rows || rows.length === 0) return null;

  let item = rows.find((row) => row.id === cleanId);
  if (item) return item;

  try {
    const decoded = decodeURIComponent(id);
    item = rows.find((row) => row.id === decoded || row.url === decoded);
    if (item) return item;
  } catch {}

  item = rows.find((row) => (row.id && id && (row.id.includes(id) || id.includes(row.id))) || (row.url && id && row.url.includes(id)));
  if (item) return item;
